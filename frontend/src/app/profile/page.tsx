...
import {
    MapPin, Trophy, BadgeCheck, Info, Users, Calendar, BarChart3,
    Phone, Mail, MessageCircle, ChevronDown, ChevronUp, Settings,
    Award, History, ArrowRightLeft, Stethoscope, Star, Medal, Crown,
    Swords, Target, Shield, CircleDot, Dumbbell, Activity, Flame, Zap, UserPlus, MessageSquare
} from 'lucide-react';
...
export default function PlayerProfilePage() {
    ...
    useEffect(() => {
        ...
        api.getMatches?.()?.then?.((matches: any[]) => {
            ...
            if (cricketMatches.length > 0) {
                ...
                cricketMatches.forEach((m: any) => {
                    const stat = m.playerStats?.find((ps: any) => ps.playerId === playerId);
                    const sd = stat?.statsData ? (typeof stat.statsData === 'string' ? JSON.parse(stat.statsData) : stat.statsData) : {};
                    totalRuns += sd.runs || 0; totalWickets += sd.wickets || 0; totalCatches += sd.catches || 0;
                    totalFours += sd.fours || 0; totalSixes += sd.sixes || 0;
                    totalStumpings += sd.stumpings || 0; totalRunOuts += sd.runOuts || 0;
                    if ((sd.runs || 0) > highScore) highScore = sd.runs || 0;
                    if ((sd.runs || 0) >= 100) totalCenturies++;
                    if ((sd.runs || 0) >= 50 && (sd.runs || 0) < 100) totalFifties++;
                });
                setCareerStats(prev => ({ ...prev, cricket: { ...EMPTY_CRICKET, matches: cricketMatches.length, innings: cricketMatches.length, runs: totalRuns, highScore, avg: Math.round((totalRuns / cricketMatches.length) * 100) / 100, wickets: totalWickets, catches: totalCatches, fours: totalFours, sixes: totalSixes, centuries: totalCenturies, fifties: totalFifties, stumpings: totalStumpings, runOuts: totalRunOuts } }));
            }
            ...
            if (footballMatches.length > 0) {
                ...
                footballMatches.forEach((m: any) => {
                    const stat = m.playerStats?.find((ps: any) => ps.playerId === playerId);
                    const sd = stat?.statsData ? (typeof stat.statsData === 'string' ? JSON.parse(stat.statsData) : stat.statsData) : {};
                    totalGoals += sd.goals || 0; totalAssists += sd.assists || 0; totalSaves += sd.saves || 0;
                    totalTackles += sd.tackles || 0; totalYellow += sd.yellowCards || 0; totalRed += sd.redCards || 0;
                    totalMinutes += sd.minutesPlayed || 0;
                });
                setCareerStats(prev => ({ ...prev, football: { ...EMPTY_FOOTBALL, matches: footballMatches.length, goals: totalGoals, assists: totalAssists, saves: totalSaves, tackles: totalTackles, yellowCards: totalYellow, redCards: totalRed, minutesPlayed: totalMinutes } }));
            }
        }).catch(() => { });
        ...
        api.getCertificates?.(playerId)?.then?.((certs: any[]) => {
            if (Array.isArray(certs)) {
                ...
                setAchievements(certs.filter((c: any) => c.type === 'WINNER' || c.type === 'AWARD' || c.type === 'MVP').map((c: any) => ({
                    ...
                    type: c.type, tournament: c.tournamentName || '—',
                })));
            }
        }).catch(() => { });
    ...
    const sportName = selectedSport?.name || profile.primarySport || playerSports[0]?.sport?.name || 'Multi-sport';
        ...
        return (
            <>
                ...
                {(isBatsman && playerRole !== 'All-Rounder') && (
                    <>
                        <button onClick={() => setShowSecondaryStats(!showSecondaryStats)} style={{ padding: '12px 18px', borderRadius: '12px', border: '1px solid #e2e8f0', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                            <span style={{ fontSize: '13px', fontWeight: 700, color: '#4f46e5' }}>{showSecondaryStats ? 'Hide' : 'Show'} Bowling & Fielding Stats</span>
                            {showSecondaryStats ? <ChevronUp size={16} color="#4f46e5" /> : <ChevronDown size={16} color="#4f46e5" />}
                        </button>
                        {showSecondaryStats && (<><StatGrid title="Bowling" icon={<Target size={16} color="#dc2626" />} stats={bowlingStats} /><StatGrid title="Fielding" icon={<Shield size={16} color="#0d9488" />} stats={fieldingStats} /></>)}
                    </>
                )}
                {playerRole === 'Bowler' && (
                    <>
                        <button onClick={() => setShowSecondaryStats(!showSecondaryStats)} style={{ padding: '12px 18px', borderRadius: '12px', border: '1px solid #e2e8f0', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                            <span style={{ fontSize: '13px', fontWeight: 700, color: '#4f46e5' }}>{showSecondaryStats ? 'Hide' : 'Show'} Batting & Fielding Stats</span>
                            {showSecondaryStats ? <ChevronUp size={16} color="#4f46e5" /> : <ChevronDown size={16} color="#4f46e5" />}
                        </button>
                        {showSecondaryStats && (<><StatGrid title="Batting" icon={<Swords size={16} color="#4f46e5" />} stats={battingStats} /><StatGrid title="Fielding" icon={<Shield size={16} color="#0d9488" />} stats={fieldingStats} /></>)}
                    </>
                )}
                {playerRole === 'All-Rounder' && (
                    <>
                        <button onClick={() => setShowSecondaryStats(!showSecondaryStats)} style={{ padding: '12px 18px', borderRadius: '12px', border: '1px solid #e2e8f0', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                            <span style={{ fontSize: '13px', fontWeight: 700, color: '#4f46e5' }}>{showSecondaryStats ? 'Hide' : 'Show'} Fielding Stats</span>
                            {showSecondaryStats ? <ChevronUp size={16} color="#4f46e5" /> : <ChevronDown size={16} color="#4f46e5" />}
                        </button>
    ...
    const renderFootballStats = () => {
        ...
        const isGK = pos === 'goalkeeper'; const isDefender = pos === 'defender';
        const isForward = pos === 'forward'; const isMidfielder = pos === 'midfielder';
        const allStats = [
    ...
    const renderOtherSportStats = () => {
        if (sportKey === 'basketball') { const bs = careerStats.basketball; return <StatGrid title="Basketball Stats" icon={<Flame size={16} color="#ea580c" />} stats={[{ label: 'Matches', value: bs.matches }, { label: 'Points', value: bs.points }, { label: 'Rebounds', value: bs.rebounds }, { label: 'Assists', value: bs.assists }, { label: 'Steals', value: bs.steals }, { label: 'Blocks', value: bs.blocks }, { label: 'Turnovers', value: bs.turnovers }, { label: 'Minutes', value: bs.minutesPlayed }]} />; }
        if (sportKey === 'kabaddi') { const ks = careerStats.kabaddi; return <StatGrid title="Kabaddi Stats" icon={<Dumbbell size={16} color="#7c3aed" />} stats={[{ label: 'Matches', value: ks.matches }, { label: 'Raid Points', value: ks.raidPoints }, { label: 'Tackle Points', value: ks.tacklePoints }, { label: 'Super Raids', value: ks.superRaids }, { label: 'Super Tackles', value: ks.superTackles }, { label: 'Total Points', value: ks.totalPoints }]} />; }
        if (sportKey === 'volleyball') { const vs = careerStats.volleyball; return <StatGrid title="Volleyball Stats" icon={<Zap size={16} color="#0ea5e9" />} stats={[{ label: 'Matches', value: vs.matches }, { label: 'Kills', value: vs.kills }, { label: 'Blocks', value: vs.blocks }, { label: 'Aces', value: vs.aces }, { label: 'Digs', value: vs.digs }, { label: 'Assists', value: vs.assists }]} />; }
        if (sportKey === 'hockey') { const hs = careerStats.hockey; return <StatGrid title="Hockey Stats" icon={<Activity size={16} color="#0d9488" />} stats={[{ label: 'Matches', value: hs.matches }, { label: 'Goals', value: hs.goals }, { label: 'Assists', value: hs.assists }, { label: 'Saves', value: hs.saves }, { label: 'Penalty Corners', value: hs.penaltyCorners }, { label: 'Yellow Cards', value: hs.yellowCards }]} />; }
        return null;
    };
            ...
            <div style={{ maxWidth: '1000px', margin: '20px auto 0', padding: '0 20px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '16px' }}>
                        ...
                        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px' }}>
                            ...
                            {(profile.teamPlayers?.length || 0) === 0 ? (
                                <div style={{ padding: '20px', textAlign: 'center' }}><Users size={36} color="#cbd5e1" /><p style={{ color: '#94a3b8', fontSize: '13px', marginTop: '10px' }}>Not part of any team yet</p></div>
                            ) : (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                    {profile.teamPlayers?.map((tp: any, i: number) => (
                                        <div key={tp.id || i} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '12px 16px', borderRadius: '10px', background: '#f8fafc' }}>
                                            <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: 'linear-gradient(135deg, #4f46e5, #7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 800, flexShrink: 0 }}>
                                                <Users size={20} />
                                            </div>
                                            <div style={{ flex: 1 }}>
                        ...
                        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px' }}>
                            ...
                            {recentMatches.length === 0 ? (
                                ...
                                recentMatches.slice(0, 5).map((m, i) => (
                                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 0', borderBottom: i < Math.min(recentMatches.length, 5) - 1 ? '1px solid #f8fafc' : 'none' }}>
                ...
                {activeTab === 'stats' && (
                    <div style={{ display: 'grid', gap: '14px' }}>
                        ...
                        <div style={{ padding: '18px', borderRadius: '14px', background: 'white', border: '1px solid #e2e8f0' }}>
                            <h3 style={{ fontSize: '14px', fontWeight: 700, color: '#0f172a', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}><BarChart3 size={16} color="#4f46e5" /> Performance Index</h3>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                                <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: `conic-gradient(#4f46e5 ${performanceIndex}%, #e2e8f0 ${performanceIndex}%)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', fontWeight: 900, color: '#4f46e5' }}>{performanceIndex}</div>
                                </div>
                                <div>
                                    <div style={{ fontSize: '14px', fontWeight: 700, color: '#1e1b4b' }}>{performanceIndex >= 80 ? 'Excellent' : performanceIndex >= 60 ? 'Good' : performanceIndex >= 40 ? 'Average' : 'Developing'}</div>
                ...
                {activeTab === 'teams' && (
                    <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px' }}>
                        ...
                        {(profile.teamPlayers?.length || 0) === 0 ? (
                            <div style={{ padding: '40px', textAlign: 'center' }}><Users size={40} color="#cbd5e1" /><p style={{ color: '#94a3b8', fontSize: '14px', marginTop: '12px' }}>Not part of any team yet</p></div>
                        ) : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                {profile.teamPlayers?.map((tp: any) => (
                                    <div key={tp.id} style={{ padding: '14px 18px', borderRadius: '12px', background: '#f8fafc', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <div>
                ...
                {activeTab === 'achievements' && (
                    <div style={{ display: 'grid', gap: '16px' }}>
                        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px' }}>
                            <h3 style={{ fontSize: '16px', fontWeight: 800, color: '#0f172a', margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: '8px' }}><Crown size={18} color="#f59e0b" /> Trophies & Championships</h3>
                        ...
                        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px' }}>
                            <h3 style={{ fontSize: '16px', fontWeight: 800, color: '#0f172a', margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: '8px' }}><Star size={18} color="#6366f1" /> Individual Awards</h3>
                            {achievements.filter(a => a.type !== 'WINNER').length === 0 ? (
                                <div style={{ padding: '30px', textAlign: 'center' }}><Medal size={40} color="#cbd5e1" /><p style={{ color: '#94a3b8', fontSize: '14px', marginTop: '12px' }}>No individual awards yet.</p></div>
                            ) : (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                    {achievements.filter(a => a.type !== 'WINNER').map((a, i) => (
                                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', borderRadius: '10px', background: '#f8fafc' }}>
                                            <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: '#eef2ff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{a.icon}</div>
                                            <div style={{ flex: 1 }}>
                        ...
                        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px' }}>
                            <h3 style={{ fontSize: '16px', fontWeight: 800, color: '#0f172a', margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: '8px' }}><BadgeCheck size={18} color="#6366f1" /> Certificates</h3>
                            {certificates.length === 0 ? (
                                <div style={{ padding: '30px', textAlign: 'center' }}><Award size={40} color="#cbd5e1" /><p style={{ color: '#94a3b8', fontSize: '14px', marginTop: '12px' }}>No certificates earned yet.</p></div>
                            ) : (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                    {certificates.map((cert, i) => (
                                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 14px', borderRadius: '10px', background: '#f8fafc' }}>
                ...
                {activeTab === 'history' && (
                    <div style={{ display: 'grid', gap: '16px' }}>
                        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px' }}>
                            <h3 style={{ fontSize: '16px', fontWeight: 800, color: '#0f172a', margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: '8px' }}><ArrowRightLeft size={18} color="#6366f1" /> Transfer History</h3>
                            {transfers.length === 0 ? (
                                <div style={{ padding: '30px', textAlign: 'center' }}><ArrowRightLeft size={40} color="#cbd5e1" /><p style={{ color: '#94a3b8', fontSize: '14px', marginTop: '12px' }}>No transfer history.</p></div>
                            ) : (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                    {transfers.map((tr, i) => (
                                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', borderRadius: '10px', background: '#f8fafc' }}>
                        ...
                        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '20px' }}>
                            <h3 style={{ fontSize: '16px', fontWeight: 800, color: '#0f172a', margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: '8px' }}><Stethoscope size={18} color="#ef4444" /> Injury Records</h3>
                            {injuries.length === 0 ? (
                                <div style={{ padding: '30px', textAlign: 'center' }}><Stethoscope size={40} color="#cbd5e1" /><p style={{ color: '#94a3b8', fontSize: '14px', marginTop: '12px' }}>No injury records. Stay healthy!</p></div>
                            ) : (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
