...
export default function VenuesPage() {
    ...
    const STATUS_CONFIG: Record<string, { bg: string; color: string; label: string }> = {
        AVAILABLE: { bg: '#ecfdf5', color: "inherit", label: '✓ Available' },
        BOOKED: { bg: '#eff6ff', color: "inherit", label: '📅 Booked' },
        MAINTENANCE: { bg: '#fffbeb', color: "inherit", label: '🔧 Maintenance' },
    };
    ...
    return (
        <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f0fdf4 0%, #dcfce7 50%, #bbf7d0 100%)' }}>
            ...
            <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '32px 24px' }}>
                <h1 style={{ fontSize: '36px', fontWeight: 900, color: "inherit", marginBottom: '8px' }}>🏟️ Venues & Facilities</h1>
                <p style={{ color: "inherit", fontSize: '16px', marginBottom: '28px' }}>Discover stadiums, arenas, and training centres across India</p>

                {/* Stats */}
                <div className="grid-cols-2-mobile" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '24px' }}>
                    {[
                        { label: 'Total Venues', value: filtered.length, icon: '🏟️', color: "inherit" },
                        { label: 'Available', value: filtered.filter(v => v.status === 'AVAILABLE').length, icon: '✅', color: "inherit" },
                        { label: 'Total Capacity', value: `${(filtered.reduce((a, v) => a + v.capacity, 0) / 1000).toFixed(1)}K`, icon: '👥', color: "inherit" },
                        { label: 'States Covered', value: new Set(filtered.map(v => v.state)).size, icon: '🗺️', color: "inherit" },
                    ].map(s => (
                ...
                <div style={{ display: 'grid', gap: '14px' }}>
                    {filtered.length === 0 && <div style={{ color: "inherit", textAlign: 'center', padding: '20px' }}>No venues found for this sport.</div>}
                    {filtered.map(v => {
                            ...
                            <div key={v.id} style={{ background: '#fff', borderRadius: '16px', padding: '24px', boxShadow: '0 2px 12px rgba(22,101,52,0.06)', display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '6px' }}>
                                        <div>
                                            <h3 style={{ fontSize: '18px', fontWeight: 800, color: "inherit", marginBottom: '2px' }}>{v.name}</h3>
                                            <div style={{ fontSize: '13px', color: "inherit" }}>📍 {v.city}, {v.state} • {v.type} • {'⭐'.repeat(Math.round(v.rating))} {v.rating}</div>
                                        </div>
                                    ...
                                    <div style={{ display: 'flex', gap: '16px', fontSize: '12px', color: "inherit", marginBottom: '8px' }}>
                                        <span>👥 Capacity: {v.capacity.toLocaleString()}</span>
                                        <span>🏷️ {v.surface}</span>
                                        <span>{v.indoor ? '🏠 Indoor' : '☀️ Outdoor'}</span>
                                    </div>
                                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '8px' }}>
                                        {v.sports.map(s => <span key={s} style={{ padding: '3px 8px', borderRadius: '6px', background: '#f0fdf4', color: "inherit", fontSize: '11px', fontWeight: 600 }}>{s}</span>)}
                                    </div>
                                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                                        {v.facilities.map(f => <span key={f} style={{ padding: '2px 6px', borderRadius: '4px', background: '#f8fafc', color: "inherit", fontSize: '10px' }}>{f}</span>)}
                                    </div>
                                </div>
                            </div>
                        );
