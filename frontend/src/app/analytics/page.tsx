...
export default function AnalyticsPage() {
    ...
    const statsCards = [
        { label: 'Total Tournaments', value: tournaments.length, icon: '🏆', color: "inherit", bg: 'linear-gradient(135deg, #eef2ff, #e0e7ff)' },
        { label: 'Active Sports', value: sports.length, icon: '⚽', color: "inherit", bg: 'linear-gradient(135deg, #ecfdf5, #d1fae5)' },
        { label: 'Registered Teams', value: teams.length, icon: '👥', color: "inherit", bg: 'linear-gradient(135deg, #fffbeb, #fef3c7)' },
        { label: 'Total Matches', value: matches.length, icon: '🏏', color: "inherit", bg: 'linear-gradient(135deg, #fef2f2, #fee2e2)' },
        { label: 'Est. Revenue', value: `₹${totalRevenue.toLocaleString()}`, icon: '💰', color: "inherit", bg: 'linear-gradient(135deg, #f5f3ff, #ede9fe)' },
        { label: 'Total Players', value: totalPlayers, icon: '🧑‍🤝‍🧑', color: "inherit", bg: 'linear-gradient(135deg, #ecfeff, #cffafe)' },
    ];
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#0f172a', color: "inherit" }}>
            ...
            <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 24px' }}>
                ...
                <p style={{ color: "inherit", fontSize: '16px', marginBottom: '32px' }}>Platform performance insights and metrics</p>
                ...
                <div className="grid-cols-2-mobile" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '32px' }}>
                    {statsCards.map((stat) => (
                        <div key={stat.label} style={{ background: 'rgba(255,255,255,0.04)', borderRadius: '16px', padding: '24px', border: '1px solid rgba(255,255,255,0.06)', transition: 'transform 0.2s, background 0.2s' }}
                            ...
                            <div style={{ fontSize: '13px', color: "inherit", marginTop: '4px', fontWeight: 600 }}>{stat.label}</div>
                        </div>
                    ))}
                </div>
                ...
                <div className="responsive-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '32px' }}>
                    ...
                    <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: '16px', padding: '24px', border: '1px solid rgba(255,255,255,0.06)' }}>
                        ...
                        {Object.entries(statusCounts).length === 0 ? (
                            <div style={{ color: "inherit", textAlign: 'center', padding: '20px' }}>No data yet</div>
                        ) : (
                                ...
                                return (
                                    <div key={status} style={{ marginBottom: '16px' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', fontWeight: 600, marginBottom: '6px' }}>
                                            ...
                                            <span style={{ color: "inherit" }}>{count as number} ({pct}%)</span>
                                        </div>
                                ...
                                {Object.entries(sportsCounts).map(([sport, count]) => {
                                    ...
                                    return (
                                        <div key={sport} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px', borderRadius: '12px', background: 'rgba(255,255,255,0.04)' }}>
                                            ...
                                            <div>
                                                <div style={{ fontWeight: 700, fontSize: '14px', color: "inherit" }}>{sport}</div>
                                                <div style={{ fontSize: '12px', color: "inherit" }}>{count as number} tournament{(count as number) > 1 ? 's' : ''}</div>
                                            </div>
                                        </div>
                                    );
                                })}
                ...
                <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: '16px', padding: '24px', border: '1px solid rgba(255,255,255,0.06)' }}>
                    ...
                    {tournaments.length === 0 ? (
                        <div style={{ color: "inherit", textAlign: 'center', padding: '20px' }}>No tournaments yet</div>
                    ) : (
                            ...
                            {tournaments.slice(0, 10).map((t: any) => {
                                ...
                                return (
                                    <Link key={t.id} href={`/tournaments/${t.id}`} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 16px', borderRadius: '12px', background: 'rgba(255,255,255,0.02)', textDecoration: 'none', color: 'inherit', transition: 'background 0.2s' }}
                                        ...
                                        <div style={{ flex: 1 }}>
                                            <div style={{ fontWeight: 700, fontSize: '15px', color: "inherit" }}>{t.name}</div>
                                            <div style={{ fontSize: '12px', color: "inherit" }}>{sport?.name || 'Sport'} · {t.format} · {t.level}</div>
                                        </div>
                ...
                <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginTop: '32px', flexWrap: 'wrap' }}>
                    {[
                        { href: '/leaderboard', label: '🏆 Leaderboard', color: "inherit" },
                        { href: '/certificates', label: '🏅 Certificates', color: "inherit" },
                        { href: '/transfers', label: '🔄 Transfers', color: "inherit" },
                    ].map((link) => (
