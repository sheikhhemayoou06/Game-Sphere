...
export default function CertificatesPage() {
    ...
    const getTypeBadge = (type: string) => {
        switch (type) {
            case 'WINNER': return { bg: 'linear-gradient(135deg, #fbbf24, #f59e0b)', color: '#78350f', icon: '🏆' };
    ...
    return (
        <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #fffbeb 0%, #fef3c7 50%, #fde68a 100%)' }}>
            ...
            <div style={{ maxWidth: '900px', margin: '0 auto', padding: '32px 24px' }}>
                ...
                {tab === 'browse' && (
                    loading ? (
                        <div style={{ textAlign: 'center', padding: '60px', color: '#92400e' }}>
                        ...
                        <div style={{ background: '#fff', borderRadius: '20px', padding: '60px', textAlign: 'center', boxShadow: '0 4px 24px rgba(120,53,15,0.1)' }}>
                            ...
                            <div style={{ fontSize: '22px', fontWeight: 800, color: '#78350f', marginBottom: '8px' }}>No Certificates Yet</div>
                            <div style={{ color: '#92400e', fontSize: '14px' }}>Certificates are generated when tournaments are completed</div>
                        </div>
                    ) : (
                            ...
                            {filteredCertificates.map((cert) => {
                                ...
                                return (
                                    <div key={cert.id} style={{ background: '#fff', borderRadius: '16px', padding: '24px', boxShadow: '0 2px 16px rgba(120,53,15,0.08)', display: 'flex', alignItems: 'center', gap: '20px', border: '1px solid rgba(120,53,15,0.08)', transition: 'transform 0.2s' }}>
                                        ...
                                        <div style={{ flex: 1 }}>
                                            <div style={{ fontWeight: 800, fontSize: '17px', color: '#1c1917', marginBottom: '4px' }}>{cert.recipientName}</div>
                                            <div style={{ fontSize: '13px', color: '#78716c' }}>{cert.tournamentName || 'Tournament'} · {cert.sportName || 'Sport'}</div>
                                            {cert.position && <div style={{ fontSize: '13px', color: '#f59e0b', fontWeight: 700, marginTop: '2px' }}>🏅 {cert.position}</div>}
                                        </div>
                                        <div style={{ textAlign: 'right' }}>
                                            ...
                                            <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '6px', fontFamily: 'monospace' }}>{cert.verificationCode?.slice(0, 8)}...</div>
                                        </div>
                                    </div>
                                );
                            })}
