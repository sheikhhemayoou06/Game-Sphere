...
export default function PaymentsPage() {
    ...
    const statusStyle = (s: string) => {
        switch (s) {
            case 'PAID': case 'RECEIVED': return { bg: '#052e16', color: "inherit" };
            case 'PENDING': return { bg: '#422006', color: "inherit" };
            case 'OVERDUE': return { bg: '#450a0a', color: "inherit" };
            ...
            <div style={{ minHeight: '100vh', background: '#faf5ff' }}>
                ...
                <div style={{ background: 'white', borderBottom: '1px solid #e9d5ff', padding: '14px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                        ...
                        <span style={{ fontWeight: 800, fontSize: '18px', color: "inherit" }}>{sportIcon} {selectedSport ? `${sportLabel} Financial Dashboard` : '💰 Financial Dashboard'}</span>
                    </div>
                ...
                <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '24px 32px' }}>
                    ...
                    <div className="grid-cols-2-mobile" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px', marginBottom: '24px' }}>
                        {[
                            { label: 'Total Revenue', value: fmt(totalRevenue), icon: '📈', color: "inherit" },
                            { label: 'Total Expenses', value: fmt(totalExpenses), icon: '📉', color: "inherit" },
                            { label: 'Net Balance', value: fmt(netBalance), icon: '💎', color: netBalance >= 0 ? '#7c3aed' : '#ef4444' },
                            { label: 'Pending Collections', value: fmt(pendingCollections), icon: '⏳', color: "inherit" },
                        ].map((s, i) => (
                            <div key={i} style={{ padding: '20px', borderRadius: '14px', background: 'white', border: '1px solid #f3e8ff', textAlign: 'center' as const }}>
                                ...
                                <div style={{ fontSize: '12px', color: "inherit", marginTop: '2px' }}>{s.label}</div>
                            </div>
                        ))}
                    </div>
                    ...
                    {ownerTab === 'revenue' && (
                        <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #f3e8ff', overflowX: 'auto' }}>
                            <div style={{ minWidth: '800px' }}>
                                <div style={{ display: 'grid', gridTemplateColumns: '0.3fr 0.6fr 0.5fr 0.4fr 0.35fr 0.35fr', padding: '14px 20px', background: '#faf5ff', fontSize: '12px', fontWeight: 700, color: "inherit", textTransform: 'uppercase' as const }}>
                                ...
                                {sportRevenue.map((r, i) => (
                                    <div key={i} style={{ display: 'grid', gridTemplateColumns: '0.3fr 0.6fr 0.5fr 0.4fr 0.35fr 0.35fr', padding: '14px 20px', borderTop: '1px solid #f3e8ff', alignItems: 'center' }}>
                                        <span style={{ fontSize: '11px', color: "inherit", fontWeight: 600 }}>{r.id}</span>
                                        <span style={{ fontWeight: 700, fontSize: '13px', color: "inherit" }}>{r.from}</span>
                                        <span style={{ fontSize: '12px', color: "inherit" }}>{r.type}</span>
                                        <span style={{ fontSize: '14px', fontWeight: 700, color: "inherit" }}>+{fmt(r.amount)}</span>
                                        ...
                                        <span style={{ fontSize: '12px', color: "inherit" }}>{r.date}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    {ownerTab === 'expenses' && (
                        <div>
                            ...
                            <div className="grid-cols-2-mobile" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '16px' }}>
                                {[
                                    { label: 'Player Acquisition', value: fmt(770000), pct: '85%', color: "inherit" },
                                    { label: 'Venue Rental', value: fmt(45000), pct: '5%', color: "inherit" },
                                    { label: 'Equipment', value: fmt(45000), pct: '5%', color: "inherit" },
                                    { label: 'Registration', value: fmt(15000), pct: '2%', color: "inherit" },
                                ].map((c, i) => (
                                    <div key={i} style={{ padding: '14px', borderRadius: '12px', background: 'white', border: '1px solid #f3e8ff' }}>
                                        <div style={{ fontSize: '11px', color: "inherit", marginBottom: '4px' }}>{c.label}</div>
                                        <div style={{ fontWeight: 800, fontSize: '18px', color: c.color }}>{c.value}</div>
                                        <div style={{ fontSize: '10px', color: "inherit" }}>{c.pct} of total</div>
                                    </div>
                                ))}
                            </div>
                            <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #f3e8ff', overflowX: 'auto' }}>
                                <div style={{ minWidth: '800px' }}>
                                    ...
                                    {sportExpenses.map((e, i) => (
                                        <div key={i} style={{ display: 'grid', gridTemplateColumns: '0.3fr 0.6fr 0.5fr 0.4fr 0.35fr 0.35fr', padding: '14px 20px', borderTop: '1px solid #f3e8ff', alignItems: 'center' }}>
                                            <span style={{ fontSize: '11px', color: "inherit", fontWeight: 600 }}>{e.id}</span>
                                            <span style={{ fontWeight: 700, fontSize: '13px', color: "inherit" }}>{e.to}</span>
                                            <span style={{ fontSize: '12px', color: "inherit" }}>{e.type}</span>
                                            <span style={{ fontSize: '14px', fontWeight: 700, color: "inherit" }}>-{fmt(e.amount)}</span>
                                            ...
                                            <span style={{ fontSize: '12px', color: "inherit" }}>{e.date}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    {ownerTab === 'invoices' && (
                        <div>
                            <div style={{ padding: '24px', borderRadius: '16px', background: 'white', border: '1px solid #f3e8ff', marginBottom: '20px' }}>
                                <h3 style={{ fontSize: '16px', fontWeight: 800, color: "inherit", marginBottom: '16px' }}>🧾 Generate Invoice</h3>
                                <div className="responsive-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '16px' }}>
                                    <div>
                                        <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: "inherit", marginBottom: '6px', textTransform: 'uppercase' as const }}>Recipient</label>
                                    ...
                                    <div>
                                        <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: "inherit", marginBottom: '6px', textTransform: 'uppercase' as const }}>Invoice Type</label>
                                        <select style={{ width: '100%', padding: '10px 14px', borderRadius: '8px', border: '1px solid #e9d5ff', fontSize: '14px', outline: 'none' }}>
                                ...
                                <div style={{ marginBottom: '14px' }}>
                                    <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: "inherit", marginBottom: '6px', textTransform: 'uppercase' as const }}>Due Date</label>
                            ...
                            {/* Invoice Templates */}
                            <h4 style={{ fontSize: '14px', fontWeight: 700, color: "inherit", marginBottom: '10px' }}>📋 Quick Templates</h4>
                            <div className="responsive-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
                                {INVOICE_TEMPLATES.map((t, i) => (
                                    <div key={i} style={{ padding: '18px', borderRadius: '14px', background: 'white', border: '1px solid #f3e8ff' }}>
                                        <div style={{ fontWeight: 700, fontSize: '15px', color: "inherit", marginBottom: '4px' }}>{t.label}</div>
                                        <div style={{ fontSize: '20px', fontWeight: 800, color: "inherit", marginBottom: '4px' }}>{fmt(t.amount)}</div>
                                        <div style={{ fontSize: '12px', color: "inherit", marginBottom: '10px' }}>{t.desc}</div>
                                        <button style={{ padding: '8px 16px', borderRadius: '8px', border: '1px solid #e9d5ff', background: 'white', color: "inherit", fontWeight: 600, cursor: 'pointer', fontSize: '12px' }}>Use Template</button>
                                    </div>
                                ))}
                            </div>
                        </div>
    ...
    return (
        <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #f0fdf4, #faf5ff, #f8fafc)' }}>
            ...
            <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '28px 32px' }}>
                <h1 style={{ fontSize: '28px', fontWeight: 900, color: "inherit", marginBottom: '6px' }}>{sportIcon} {selectedSport ? `${sportLabel} Payments & Transactions` : '💳 Payments & Transactions'}</h1>
                <p style={{ color: "inherit", marginBottom: '20px', fontSize: '14px' }}>{selectedSport ? `${sportLabel} payments, receipts, and dues` : 'Track all payments, download receipts, and manage dues'}</p>

                {/* Stats */}
                <div className="grid-cols-2-mobile" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px', marginBottom: '20px' }}>
                    {[
                        { label: 'Total Paid', value: fmt(sportPayments.filter(p => p.status === 'PAID').reduce((s, p) => s + p.amount, 0)), icon: '✅', color: "inherit" },
                        { label: 'Pending', value: fmt(sportPayments.filter(p => p.status === 'PENDING').reduce((s, p) => s + p.amount, 0)), icon: '⏳', color: "inherit" },
                        { label: 'Overdue', value: fmt(sportPayments.filter(p => p.status === 'OVERDUE').reduce((s, p) => s + p.amount, 0)), icon: '🚨', color: "inherit" },
                        { label: 'Transactions', value: sportPayments.length, icon: '📊', color: "inherit" },
                    ].map((s, i) => (
                ...
                <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #f1f5f9', overflowX: 'auto', marginBottom: '16px' }}>
                    <div style={{ minWidth: '800px' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '0.3fr 0.5fr 0.7fr 0.35fr 0.3fr 0.35fr 0.3fr', padding: '12px 18px', background: '#f8fafc', fontSize: '11px', fontWeight: 700, color: "inherit", textTransform: 'uppercase' as const }}>
                        ...
                        {filtered.map((p, i) => (
                            <div key={i} style={{ display: 'grid', gridTemplateColumns: '0.3fr 0.5fr 0.7fr 0.35fr 0.3fr 0.35fr 0.3fr', padding: '12px 18px', borderTop: '1px solid #f1f5f9', alignItems: 'center' }}>
                                <span style={{ fontSize: '11px', color: "inherit", fontWeight: 600 }}>{p.id}</span>
                                <span style={{ fontSize: '12px', color: "inherit", fontWeight: 600 }}>{p.type}</span>
                                <span style={{ fontSize: '12px', color: "inherit" }}>{p.desc}</span>
                                <span style={{ fontSize: '14px', fontWeight: 700, color: "inherit" }}>{fmt(p.amount)}</span>
                                ...
                                <span style={{ fontSize: '12px', color: "inherit" }}>{p.date}</span>
                                <div>
                                    {p.status === 'PAID' ? (
                                        <button onClick={() => setShowReceipt(p.id)} style={{ padding: '4px 10px', borderRadius: '6px', border: '1px solid #e2e8f0', background: 'white', color: "inherit", fontSize: '11px', fontWeight: 600, cursor: 'pointer' }}>🧾 Receipt</button>
                                    ) : (
                ...
                {/* Payment Methods */}
                <h3 style={{ fontSize: '14px', fontWeight: 700, color: "inherit", marginBottom: '10px' }}>💳 Payment Methods</h3>
                <div className="flex-wrap-mobile" style={{ display: 'flex', gap: '12px' }}>
                    {PAYMENT_METHODS.map((m, i) => (
                        <div key={i} style={{ padding: '14px 18px', borderRadius: '12px', background: 'white', border: m.primary ? '2px solid #14b8a6' : '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '10px' }}>
                            ...
                            <div>
                                <div style={{ fontWeight: 700, fontSize: '13px', color: "inherit" }}>{m.name}</div>
                                <div style={{ fontSize: '11px', color: "inherit" }}>{m.details}</div>
                            </div>
                            {m.primary && <span style={{ padding: '2px 8px', borderRadius: '4px', background: '#f0fdf4', color: "inherit", fontSize: '10px', fontWeight: 700 }}>PRIMARY</span>}
                        </div>
                    ))}
                </div>
            </div>
                    ...
                    <div onClick={(e) => e.stopPropagation()} style={{ width: '420px', padding: '28px', borderRadius: '16px', background: 'white' }}>
                        <div style={{ textAlign: 'center' as const, marginBottom: '16px' }}>
                            <div style={{ marginBottom: '4px' }}>
                                <img src="/logo.png" alt="Logo" style={{ width: "auto", height: "80px", objectFit: 'contain' }} />
                            </div>
                            <div style={{ fontWeight: 800, fontSize: '18px', color: "inherit" }}>Game Sphere</div>
                            <div style={{ fontSize: '11px', color: "inherit" }}>Payment Receipt</div>
                        </div>
                        <div style={{ borderTop: '2px dashed #e2e8f0', borderBottom: '2px dashed #e2e8f0', padding: '16px 0', marginBottom: '16px' }}>
                            {[
                                ...
                                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', fontSize: '13px' }}>
                                    <span style={{ color: "inherit" }}>{r.label}</span>
                                    <span style={{ fontWeight: 600, color: "inherit" }}>{r.value}</span>
                                </div>
                            ))}
                        </div>
                        <div style={{ textAlign: 'center' as const }}>
                            <div style={{ fontSize: '28px', marginBottom: '4px' }}>✅</div>
                            <div style={{ fontSize: '12px', color: "inherit", fontWeight: 700 }}>Digitally Verified</div>
                        </div>
                        <div style={{ display: 'flex', gap: '8px', marginTop: '16px' }}>
                            <button onClick={() => window.print()} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: '1px solid #e2e8f0', background: 'white', color: "inherit", fontWeight: 600, cursor: 'pointer', fontSize: '13px' }}>🖨️ Print</button>
